    ...
    <div className='flex items-center gap-2 p-2'>
        ...
        <div className="flex justify-between items-center w-full">
            ...
            <div className="flex gap-1 items-center">
                ...
                <Button variant={"ghost"} size={"icon"} className='cursor-pointer hover:text-red-500'
                onClick={() => deleteElements({
                    nodes: [{id: nodeId}]
                })}>
                    <TrashIcon size={12} className=''/>
                </Button>
                <Button variant={"ghost"} size={"icon"} className='cursor-pointer hover:text-blue-500'
                onClick={() => {
                    const node = getNode(nodeId) as AppNode;
                    const newX = node.position.x;
                    const newY = node.position.y + node.measured?.height! + 50;
