    ...
    async def get_guest_account_stats(self) -> dict:
        ...
        try:
            ...
            async with SessionLocal() as session:
                ...
                converted_sessions_result = await session.execute(converted_sessions_stmt)
                ...
                return {
                    ...
                    "conversion_rate": (
                        converted_sessions_count / (converted_sessions_count + active_sessions_count)
                        if (converted_sessions_count + active_sessions_count) > 0 else 0
                    )
                }
