...
class HybridRateLimiter:
    ...
    def _get_limiter_key(self, request: Request) -> str:
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            endpoint_type = self._get_endpoint_category(request.url.path)
            return (
                f"user:{token[:10]}:{endpoint_type}"  # Use first 10 chars as identifier
            )
    ...
    async def check_guest_specific_limits(
        ...
        if user.guest_expires_at and user.guest_expires_at.astimezone(
            tz=UTC
        ) < datetime.now(tz=UTC):
            raise HTTPException(
                status_code=403,
...
def create_rate_limit_decorator(endpoint_type: str = "default"):
    ...
    def decorator(func):
        ...
        guest_limit = hybrid_limiter.guest_limits.get(
            endpoint_type, hybrid_limiter.guest_limits["default"]
        )
        user_limit = hybrid_limiter.user_limits.get(
            endpoint_type, hybrid_limiter.user_limits["default"]
        )
...
async def check_hybrid_rate_limit(
    request: Request, session: AsyncSession, user: Optional[User] = None
) -> bool:
    """
