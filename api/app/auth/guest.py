...
class GuestAuthManager:
    ...
    async def create_guest_user(
        ...
        try:
            ...
            guest_user = User(
                ...
                guest_expires_at=datetime.utcnow()
                + timedelta(days=self.GUEST_SESSION_DURATION_DAYS),
            )

            # Create initial balance
            balance = UserBalance(
                user_id=guest_user.id, credits=self.GUEST_INITIAL_CREDITS
            )
            ...
            guest_session = GuestSession(
                ...
                or datetime.utcnow() + timedelta(days=self.GUEST_SESSION_DURATION_DAYS),
                is_active=True,
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to create guest user: {str(e)}"
            )
    ...
    async def convert_guest_to_user(
        ...
        stmt = select(GuestSession).where(GuestSession.user_id == guest_user.id)
        result = await session.execute(stmt)
    ...
    async def cleanup_expired_guests(self, session: AsyncSession) -> int:
        ...
        stmt = select(GuestSession).where(GuestSession.expires_at < datetime.utcnow())
        result = await session.execute(stmt)
